...
pedi <- read.csv("Pedigree/pedigree.csv")
pedi_p <- prepPed(pedi)
...
tab_QG_df <- tab_QG_df %>% group_by(geneid) %>% mutate(CPM_sc = scale(log(CPM)))
...
# Fit a test animal model with one gene
f1 <- bf(CPM_sc ~ 1 + (1 | gr(ID, cov = relat_m)) + # Fits matrix w/ meth partitions for GE
       Par + Dev + (1 | Dam) + (1 | Sire)) # Env. + dam/sire effects on GE

brms_test <- brm(f1, 
  data = filter(tab_QG_df, geneid == "gene-LOC100892942"),
...
# Plot VA
mcmc_areas(brms_test,
           pars = c("sd_ID__Intercept"),
           prob = 0.95) +
  labs(x = expression(italic(V)["A"]), y = "Density")

# Export VA then calculate and plot h2
Var.table <- as_draws_df(brms_test)

v_r <- (VarCorr(brms_test, summary = FALSE)$residual$sd)^2

Var.table$h.bwt <- as.mcmc((Var.table$`sd_ID__Intercept`)^2 / 
                               ((Var.table$`sd_ID__Intercept`)^2 + 
                                  (Var.table$sd_Dam__Intercept)^2 + 
                                  (Var.table$sd_Sire__Intercept)^2 +
                                  (Var.table$sigma)^2))

# Create df of h2 measures
h2 <- as.data.frame(rbind(
  cbind(summary(Var.table$h.bwt)$statistics[1], summary(Var.table$h.bwt)$quantiles[1], summary(Var.table$h.bwt)$quantiles[5])))
...
tab_exp_df$Sibship_geneid <- paste(gsub("_.*", "", tab_exp_df$variable),
                                   tab_exp_df$geneid,
                                   sep = "_")

tab_DevN <- filter(tab_exp_df, Dev == "N")
tab_DevU <- filter(tab_exp_df, Dev == "U")

Sib_FC_df <- data.frame(Sibship_geneid = tab_DevN$Sibship_geneid,
                       CPM_N = tab_DevN$CPM,
                       CPM_U = tab_DevU$CPM)
